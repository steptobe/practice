...
YAHOO.namespace = function() {
	...
	for (b = 0; b < c.length; b += 1) {
		a = ("" + c[b]).split(".");
		e = YAHOO;
		for (d = a[0] == "YAHOO" ? 1 : 0; d < a.length; d += 1) e[a[d]] = e[a[d]] || {},
		e = e[a[d]]
	}
	return e
};
...
YAHOO.env.ua = function() {
	...
	if (b) {
		if (/windows|win32/i.test(b)) e.os = "windows";
		else if (/macintosh/i.test(b)) e.os = "macintosh";
		if (/KHTML/.test(b)) e.webkit = 1;
		if ((d = b.match(/AppleWebKit\/([^\s]*)/)) && d[1]) {
			e.webkit = c(d[1]);
			if (/ Mobile\//.test(b)) e.mobile = "Apple";
			else if (d = b.match(/NokiaN[^\/]*/)) e.mobile = d[0];
		...
		} else if ((d = b.match(/MSIE\s([^;]*)/)) && d[1]) e.ie = c(d[1]);
		else if (d = b.match(/Gecko\/([^\s]*)/)) if (e.gecko = 1, (d = b.match(/rv:([^\s\)]*)/)) && d[1]) e.gecko = c(d[1])
	}
	return e
} (); (function() {
...
YAHOO.lang = YAHOO.lang || {}; (function() {
	...
	a = {
		...
		function(a, b) {
			var k, g, j;
			for (k = 0; k < d.length; k += 1) g = d[k],
			j = b[g],
			c.isFunction(j) && j != e[g] && (a[g] = j)
		}: function() {},
		extend: function(a, b, d) {
			...
			if (d) {
				for (j in d) c.hasOwnProperty(d, j) && (a.prototype[j] = d[j]);
		...
		augmentObject: function(a, b) {
			...
			var d = arguments,
			g, j = d[2];
			if (j && j !== !0) for (g = 2; g < d.length; g += 1) a[d[g]] = b[d[g]];
			else {
		...
		dump: function(a, b) {
			var d, g, j = [];
			if (c.isObject(a)) if (a instanceof Date || "nodeType" in a && "tagName" in a) return a;
			else {
			...
			if (c.isArray(a)) {
				j.push("[");
				for (d = 0, g = a.length; d < g; d += 1) c.isObject(a[d]) ? j.push(b > 0 ? c.dump(a[d], b - 1) : "{...}") : j.push(a[d]),
				j.push(", ");
				j.length > 1 && j.pop();
				j.push("]")
			} else {
		...
		substitute: function(a, b, d) {
			for (var g, j, e, n, p, o = [], m;;) {
				...
				e = b[n];
				d && (e = d(n, e, p));
				c.isObject(e) ? c.isArray(e) ? e = c.dump(e, parseInt(p, 10)) : (p = p || "", n = p.indexOf("dump"), n > -1 && (p = p.substring(4)), m = e.toString(), e = m === "[object Object]" || n > -1 ? c.dump(e, parseInt(p, 10)) : m) : !c.isString(e) && !c.isNumber(e) && (e = "~-" + o.length + "-~", o[o.length] = m);
...
YAHOO.register("yahoo", YAHOO, {
	...
	o = b.ie;
	c.Dom = {
		CUSTOM_ATTRIBUTES: !j.hasAttribute ? {
			"for": "htmlFor",
			"class": "className"
		}: {
			htmlFor: "for",
		...
		get: function(a) {
			...
			if (a) {
				...
				if (typeof a === "string") {
					b = a;
					f = (a = g.getElementById(a)) ? a.attributes: null;
		...
		getComputedStyle: function(a, b) {
			...
			else if (a.currentStyle) return c.Dom.IE_ComputedStyle.get(a, b)
		},
		...
		_getStyle: function() {
			if (window.getComputedStyle) return function(a, b) {
				var b = b === "float" ? b = "cssFloat": c.Dom._toCamel(b),
				d = a.style[b],
				f;
				...
				case "float":
					b = "styleFloat";
				default:
					return b = c.Dom._toCamel(b),
					d = a.currentStyle ? a.currentStyle[b] : null,
					a.style[b] || d
				}
			}
		} (),
		setStyle: function(a, b, d) {
			c.Dom.batch(a, c.Dom._setStyle, {
				prop: b,
				val: d
			})
		},
		...
		_getXY: function() {
			return g.documentElement.getBoundingClientRect ?
			function(a) {
				...
				if (c.Dom._canPosition(a)) {
					...
					if (o && b.ie < 8) g = e = 2,
					h = j.compatMode,
					b.ie === 6 && h !== "BackCompat" && (g = e = 0),
					h === "BackCompat" && (h = m(j.documentElement, "borderLeftWidth"), j = m(j.documentElement, "borderTopWidth"), h !== "medium" && (e = parseInt(h, 10)), j !== "medium" && (g = parseInt(j, 10))),
					f[0] -= e,
					f[1] -= g;
					if (d || a) f[0] += a,
					f[1] += d;
				...
				if (c.Dom._canPosition(a)) {
					...
					for (j = p || b.webkit > 519 ? !0 : !1; g = g.offsetParent;) e[0] += g.offsetLeft,
					e[1] += g.offsetTop,
					j && (e = c.Dom._calcBorders(g, e));
					if (c.Dom._getStyle(a, "position") !== "fixed") {
						for (g = a; (g = g.parentNode) && g.tagName;) if (a = g.scrollTop, j = g.scrollLeft, p && c.Dom._getStyle(g, "overflow") !== "visible" && (e = c.Dom._calcBorders(g, e)), a || j) e[0] -= j,
						e[1] -= a;
						e[0] += d;
						e[1] += f
					} else if (l) e[0] -= d,
		...
		setXY: function(a, b, d) {
			c.Dom.batch(a, c.Dom._setXY, {
				pos: b,
				noRetry: d
			})
		},
		_setXY: function(a, b) {
			var d = c.Dom._getStyle(a, "position"),
			f = c.Dom.setStyle,
			j = b.pos,
			e = b.noRetry,
			...
			j[0] !== null && f(a, "left", j[0] - h[0] + g[0] + "px");
		...
		_removeClass: function(a, b) {
			var f = !1,
			j, e;
			a && b && (j = c.Dom._getAttribute(a, "className") || "", c.Dom.setAttribute(a, "className", j.replace(c.Dom._getClassRegex(b), "")), e = c.Dom._getAttribute(a, "className"), j !== e && (c.Dom.setAttribute(a, "className", d(e)), f = !0, c.Dom._getAttribute(a, "className") === "" && (j = a.hasAttribute && a.hasAttribute("class") ? "class": "className", a.removeAttribute(j))));
			return f
		},
		replaceClass: function(a, b, d) {
			return c.Dom.batch(a, c.Dom._replaceClass, {
				from: b,
				to: d
			})
		},
		_replaceClass: function(a, b) {
			var f, j, e = !1;
			if (a && b) f = b.from,
			(j = b.to) ? f ? f !== j && (e = c.Dom._getAttribute(a, "className") || "", f = (" " + e.replace(c.Dom._getClassRegex(f), " " + j)).split(c.Dom._getClassRegex(j)), f.splice(1, 0, " " + j), c.Dom.setAttribute(a, "className", d(f.join(""))), e = !0) : e = c.Dom._addClass(a, b.to) : e = !1;
			return e
		},
		...
		getAncestorByClassName: function(a, b) {
			a = c.Dom.get(a);
			return ! a ? null: c.Dom.getAncestorBy(a,
			function(a) {
		...
		getChildrenBy: function(a, b) {
			var d = c.Dom.getFirstChildBy(a, b),
			f = d ? [d] : [];
		...
		getClientRegion: function() {
			var a = c.Dom.getDocumentScrollTop(),
			b = c.Dom.getDocumentScrollLeft(),
			d = c.Dom.getViewportWidth() + b,
			f = c.Dom.getViewportHeight() + a;
			return new c.Region(a, d, f, b)
		},
		setAttribute: function(a, b, d) {
			c.Dom.batch(a, c.Dom._setAttribute, {
				attr: b,
				val: d
			})
		},
		_setAttribute: function(a, b) {
			var d = c.Dom._toCamel(b.attr),
			f = b.val;
			a && a.setAttribute && (c.Dom.DOT_ATTRIBUTES[d] ? a[d] = f: (d = c.Dom.CUSTOM_ATTRIBUTES[d] || d, a.setAttribute(d, f)))
		},
		...
		_getClassRegex: function(a) {
			var b;
			a !== void 0 && (a.exec ? b = a: (b = f[a], b || (a = a.replace(c.Dom._patterns.CLASS_RE_TOKENS, "\\$1"), b = f[a] = RegExp("(?:^|\\s)" + a + "(?= |$)", "g"))));
			return b
		},
		_patterns: {
...
YAHOO.extend(YAHOO.util.Point, YAHOO.util.Region); (function() {
	var c = YAHOO.util,
	e = /^width|height$/,
	b = /^(\d[.\d]*)+(em|ex|px|gd|rem|vw|vh|vm|ch|mm|cm|in|pt|pc|deg|rad|ms|s|hz|khz|%){1}?/i,
	d = {
		get: function(a, d) {
			var e = "",
			e = a.currentStyle[d];
		...
		getOffset: function(a, b) {
			...
			l = "pixel" + c,
			c = "";
			d == "auto" ? (c = d = a[j], e.test(b) && (a.style[b] = d, a[j] > d && (c = d - (a[j] - d)), a.style[b] = "auto")) : (!a.style[l] && !a.style[b] && (a.style[b] = d), c = a.style[l]);
			return c + "px"
		},
		...
		getBorderColor: function(a, b) {
			...
			return c.Dom.Color.toRGB(c.Dom.Color.toHex(d[b] || d.color))
		}
	},
	a = {};
	...
	b.Dom.Color = {
		...
		toRGB: function(d) {
			...
			b.Dom.Color.re_hex.exec(d) && (d = "rgb(" + [c(e.$1, 16), c(e.$2, 16), c(e.$3, 16)].join(", ") + ")");
			return d
		},
		toHex: function(d) {
...
YAHOO.util.CustomEvent.prototype = {
	...
	unsubscribe: function(c, e) {
		...
		for (var b = !1,
		d = 0,
		a = this.subscribers.length; d < a; ++d) {
	...
	fire: function() {
		...
		b = !0,
		d;
		if (this.fireOnce) if (this.fired) return ! 0;
		else this.firedWith = e;
		this.fired = !0;
...
if (!YAHOO.util.Event) YAHOO.util.Event = function() {
	...
	return {
		...
		_dri: null,
		_specialTypes: {
			focusin: k ? "focusin": "focus",
			focusout: k ? "focusout": "blur"
		},
		DOMReady: !1,
		throwErrors: !1,
		startInterval: function() {
			if (!this._interval) this._interval = YAHOO.lang.later(this.POLL_INTERVAL, this, this._tryPreloadAttach, null, !0)
		},
		...
		_addListener: function(a, d, c, f, h, k) {
			...
			if (this._isValidCollection(a)) {
				for (var m = !0,
				r = 0,
				s = a.length; r < s; ++r) m = this.on(a[r], d, c, f, h) && m;
				return m
			} else if (YAHOO.lang.isString(a)) if (m = this.getEl(a)) a = m;
			else return this.onAvailable(a,
			function() {
				YAHOO.util.Event._addListener(a, d, c, f, h, k)
			}),
			!0;
			if (!a) return ! 1;
			if ("unload" == d && f !== this) return b[b.length] = [a, d, c, f, h],
			!0;
			var q = a;
			h && (q = h === !0 ? f: h);
			m = function(b) {
		...
		removeListener: function(a, d, c, f) {
			var h, d = this._getType(d);
			if (typeof a == "string") a = this.getEl(a);
			else if (this._isValidCollection(a)) {
				f = !0;
			...
			if ("unload" == d) {
				for (h = b.length - 1; h > -1; h--) if ((f = b[h]) && f[0] == a && f[1] == d && f[2] == c) return b.splice(h, 1),
				!0;
				return ! 1
			}
			h = null;
			...
			try {
				this._simpleRemove(a, d, h[this.WFN], c)
			} catch(k) {
				return this.lastError = k,
				!1
			}
			delete e[f][this.WFN];
			delete e[f][this.FN];
		...
		getPageX: function(a) {
			var b = a.pageX; ! b && 0 !== b && (b = a.clientX || 0, this.isIE && (b += this._getScrollLeft()));
			return b
		},
		getPageY: function(a) {
			var b = a.pageY; ! b && 0 !== b && (b = a.clientY || 0, this.isIE && (b += this._getScrollTop()));
			return b
		},
		getXY: function(a) {
		...
		_ready: function() {
			var a = YAHOO.util.Event;
			if (!a.DOMReady) a.DOMReady = !0,
			a.DOMReadyEvent.fire(),
			a._simpleRemove(document, "DOMContentLoaded", a._ready)
		},
		_tryPreloadAttach: function() {
			if (a.length === 0) {
				if (d = 0, this._interval) this._interval.cancel(),
				this._interval = null
			} else if (!this.locked) if (this.isIE && !this.DOMReady) this.startInterval();
			else {
				this.locked = !0;
				var b = !c;
				b || (b = d > 0 && a.length > 0);
				...
				for (h = 0, k = a.length; h < k; h += 1) if (o = a[h]) if (m = this.getEl(o.id)) if (o.checkReady) {
					if (c || m.nextSibling || !b) r.push(o),
					a[h] = null
				} else e(m, o),
				a[h] = null;
				else f.push(o);
				for (h = 0, k = r.length; h < k; h += 1) o = r[h],
				e(this.getEl(o.id), o);
				d--;
				if (b) {
					for (h = a.length - 1; h > -1; h--) o = a[h],
					(!o || !o.id) && a.splice(h, 1);
					this.startInterval()
		...
		purgeElement: function(a, b, d) {
			var a = YAHOO.lang.isString(a) ? this.getEl(a) : a,
			c = this.getListeners(a, d),
			f;
			if (c) for (f = c.length - 1; f > -1; f--) {
				var e = c[f];
				this.removeListener(a, e.type, e.fn)
			}
			if (b && a && a.childNodes) for (f = 0, c = a.childNodes.length; f < c; ++f) this.purgeElement(a.childNodes[f], b, d)
		},
		...
		_unload: function(a) {
			...
			c, f, h, k = b.slice(),
			m;
			for (c = 0, h = b.length; c < h; ++c) if (f = k[c]) m = window,
			f[d.ADJ_SCOPE] && (m = f[d.ADJ_SCOPE] === !0 ? f[d.UNLOAD_OBJ] : f[d.ADJ_SCOPE]),
			f[d.FN].call(m, d.getEvent(a, f[d.EL]), f[d.UNLOAD_OBJ]),
			k[c] = null;
			b = null;
...
function() {
	...
	if (c.isIE) if (self !== self.top) document.onreadystatechange = function() {
		if (document.readyState == "complete") document.onreadystatechange = null,
		c._ready()
	};
	else {
		...
		c.POLL_INTERVAL)
	} else c.webkit && c.webkit < 525 ? c._dri = setInterval(function() {
		var b = document.readyState;
		if ("loaded" == b || "complete" == b) clearInterval(c._dri),
		c._dri = null,
		c._ready()
	},
...
YAHOO.util.EventProvider.prototype = {
	...
	subscribe: function(c, e, b, d) {
		this.__yui_events = this.__yui_events || {};
		var a = this.__yui_events[c];
		a ? a.subscribe(e, b, d) : (a = this.__yui_subscribers = this.__yui_subscribers || {},
		a[c] || (a[c] = []), a[c].push({
			fn: e,
			obj: b,
			overrideContext: d
	...
	unsubscribe: function(c, e, b) {
		...
		if (c) {
			...
			var c = !0,
			a;
			for (a in d) YAHOO.lang.hasOwnProperty(d, a) && (c = c && d[a].unsubscribe(e, b));
			return c
		}
		return ! 1
	},
	...
	createEvent: function(c, e) {
		...
		d = this.__yui_events,
		a;
		if (!d[c] && (a = new YAHOO.util.CustomEvent(c, b.scope || this, b.silent, YAHOO.util.CustomEvent.FLAT, b.fireOnce), d[c] = a, b.onSubscribeCallback && a.subscribeEvent.subscribe(b.onSubscribeCallback), this.__yui_subscribers = this.__yui_subscribers || {},
		b = this.__yui_subscribers[c])) for (var f = 0; f < b.length; ++f) a.subscribe(b[f].fn, b[f].obj, b[f].overrideContext);
		return d[c]
	},
...
if (!YAHOO.util.DragDropMgr) YAHOO.util.DragDropMgr = function() {
	...
	return {
		...
		_createShim: function() {
			...
			this._shim = b;
			c.on(b, "mouseup", this.handleMouseUp, this, !0);
		...
		startDrag: function(b, d) {
			if (this.dragCurrent && this.dragCurrent.useShim) this._shimState = this.useShim,
			this.useShim = !0;
			this._activateShim();
			clearTimeout(this.clickTimeout);
			var a = this.dragCurrent;
			a && a.events.b4StartDrag && (a.b4StartDrag(b, d), a.fireEvent("b4StartDragEvent", {
				x: b,
				y: d
			}));
			a && a.events.startDrag && (a.startDrag(b, d), a.fireEvent("startDragEvent", {
				x: b,
				y: d
			}));
		...
		handleMouseUp: function(b) {
			if (this.dragCurrent) {
				...
				if (this.dragThreshMet) {
					if (this.fromTimeout) this.fromTimeout = !1,
		...
		stopDrag: function(b, d) {
			var a = this.dragCurrent;
			a && !d && (this.dragThreshMet && (a.events.b4EndDrag && (a.b4EndDrag(b), a.fireEvent("b4EndDragEvent", {
				e: b
			})), a.events.endDrag && (a.endDrag(b), a.fireEvent("endDragEvent", {
				e: b
			}))), a.events.mouseUp && (a.onMouseUp(b), a.fireEvent("mouseUpEvent", {
				e: b
			})));
			if (this._shimActive && (this._deactivateShim(), this.dragCurrent && this.dragCurrent.useShim)) this.useShim = this._shimState,
			this._shimState = !1;
		...
		handleMouseMove: function(b) {
			...
			if (d) {
				if (YAHOO.util.Event.isIE && !b.button) return this.stopEvent(b),
				this.handleMouseUp(b);
				if (!this.dragThreshMet) {
					var a = Math.abs(this.startX - YAHOO.util.Event.getPageX(b)),
					c = Math.abs(this.startY - YAHOO.util.Event.getPageY(b)); (a > this.clickPixelThresh || c > this.clickPixelThresh) && this.startDrag(this.startX, this.startY)
				}
				this.dragThreshMet && (d && d.events.b4Drag && (d.b4Drag(b), d.fireEvent("b4DragEvent", {
					e: b
				})), d && d.events.drag && (d.onDrag(b), d.fireEvent("dragEvent", {
					e: b
				})), d && this.fireEvents(b, !1));
				this.stopEvent(b)
			}
		},
		fireEvents: function(b, d) {
			...
			if (a && !a.isLocked() && !a.dragOnly) {
				var c = YAHOO.util.Event.getPageX(b),
				e = YAHOO.util.Event.getPageY(b),
				k = new YAHOO.util.Point(c, e),
				e = a.getTargetCoord(k.x, k.y),
				g = a.getDragEl(),
				c = ["out", "over", "drop", "enter"],
				j = new YAHOO.util.Region(e.y, e.x + g.offsetWidth, e.y + g.offsetHeight, e.x),
				l = [],
				n = {},
				e = [],
				g = {
				...
				for (var m in a.groups) if ("string" == typeof m) for (p in this.ids[m]) o = this.ids[m][p],
				this.isTypeOfDD(o) && o.isTarget && !o.isLocked() && o != a && this.isOverTarget(k, o, this.mode, j) && (n[m] = !0, d ? g.dropEvts.push(o) : (l[o.id] ? g.overEvts.push(o) : g.enterEvts.push(o), this.dragOvers[o.id] = o));
				...
				if (d && !g.dropEvts.length) this.interactionInfo.validDrop = !1,
				a.events.invalidDrop && (a.onInvalidDrop(b), a.fireEvent("invalidDropEvent", {
					e: b
				}));
				...
				a.events[l] && (a[r](b, m, e), a.fireEvent(j, {
					...
					a.events[l] && (a[r](b, m[n].id, e[0]), a.fireEvent(j, {
						event: b,
						info: m[n].id,
						group: e[0]
					}))
		...
		isOverTarget: function(b, d, a, c) {
			...
			return (e = c.intersect(e)) ? (d.overlap = e, a ? !0 : d.cursorIsOver) : !1
		},
		_onUnload: function() {
			this.unregAll()
		},
		unregAll: function() {
			if (this.dragCurrent) this.stopDrag(),
			this.dragCurrent = null;
		...
		getScroll: function() {
			var b, d, a = document.documentElement,
			c = document.body;
			if (a && (a.scrollTop || a.scrollLeft)) b = a.scrollTop,
			d = a.scrollLeft;
			else if (c) b = c.scrollTop,
			d = c.scrollLeft;
			return {
				top: b,
				left: d
		...
		handleWasClicked: function(b, d) {
			...
			else for (var a = b.parentNode; a;) if (this.isHandle(d, a.id)) return ! 0;
			else a = a.parentNode;
			return ! 1
		}
	}
...
YAHOO.util.DDM._addListeners(); (function() {
	var c = YAHOO.util.Event,
	e = YAHOO.util.Dom;
	...
	YAHOO.util.DragDrop.prototype = {
		...
		initTarget: function(b, d, a) {
			...
			this.groups = {};
			if (typeof b !== "string") this._domRef = b,
			b = e.generateId(b);
			this.id = b;
		...
		handleMouseDown: function(b) {
			...
			if (! (this.primaryButtonOnly && d > 1) && !this.isLocked()) {
				var d = this.b4MouseDown(b),
				a = !0;
				this.events.b4MouseDown && (a = this.fireEvent("b4MouseDownEvent", b));
				var e = this.onMouseDown(b),
				h = !0;
				this.events.mouseDown && (h = this.fireEvent("mouseDownEvent", b));
				if (! (d === !1 || e === !1 || a === !1 || h === !1)) if (this.DDM.refreshCache(this.groups), d = new YAHOO.util.Point(c.getPageX(b), c.getPageY(b)), (this.hasOuterHandles || this.DDM.isOverTarget(d, this)) && this.clickValidator(b)) this.setStartPosition(),
		...
		removeInvalidHandleClass: function(b) {
			for (var d = 0,
			a = this.invalidHandleClasses.length; d < a; ++d) this.invalidHandleClasses[d] == b && delete this.invalidHandleClasses[d]
		},
		...
		setXTicks: function(b, d) {
			...
			this.xTickSize = d;
			for (var a = {},
			c = this.initPageX; c >= this.minX; c -= d) a[c] || (this.xTicks[this.xTicks.length] = c, a[c] = !0);
		...
		setYTicks: function(b, d) {
			...
			this.yTickSize = d;
			for (var a = {},
			c = this.initPageY; c >= this.minY; c -= d) a[c] || (this.yTicks[this.yTicks.length] = c, a[c] = !0);
			for (c = this.initPageY; c <= this.maxY; c += d) a[c] || (this.yTicks[this.yTicks.length] = c, a[c] = !0);
...
YAHOO.extend(YAHOO.util.DD, YAHOO.util.DragDrop, {
	...
	alignElWithMouse: function(c, e, b) {
		...
		this.deltaSetXY ? (YAHOO.util.Dom.setStyle(c, "left", d.x + this.deltaSetXY[0] + "px"), YAHOO.util.Dom.setStyle(c, "top", d.y + this.deltaSetXY[1] + "px")) : (YAHOO.util.Dom.setXY(c, [d.x, d.y]), e = parseInt(YAHOO.util.Dom.getStyle(c, "left"), 10), b = parseInt(YAHOO.util.Dom.getStyle(c, "top"), 10), this.deltaSetXY = [e - d.x, b - d.y]);
	...
	autoScroll: function(c, e, b, d) {
		if (this.scroll) {
			var a = this.DDM.getClientHeight(),
			f = this.DDM.getClientWidth(),
			h = this.DDM.getScrollTop(),
			k = this.DDM.getScrollLeft();
			d += c;
			var g = a + h - e - this.deltaY,
			j = f + k - c - this.deltaX,
			l = document.all ? 80 : 30;
...
YAHOO.extend(YAHOO.util.DDProxy, YAHOO.util.DD, {
	...
	_resizeProxy: function() {
		if (this.resizeFrame) {
			...
			e = this.getEl(),
			b = this.getDragEl(),
			d = parseInt(c.getStyle(b, "borderTopWidth"), 10),
			a = parseInt(c.getStyle(b, "borderRightWidth"), 10),
...
YAHOO.register("dragdrop", YAHOO.util.DragDropMgr, {
	...
	YAHOO.lang.augmentObject(c, {
		...
		getSliderRegion: function(a, b, d, e, g, j, l) {
			return new c(a, a, new YAHOO.widget.SliderThumb(b, a, d, e, g, j, l), "region")
		},
		SOURCE_UI_EVENT: 1,
		SOURCE_SET_VALUE: 2,
		SOURCE_KEY_EVENT: 3,
	...
	YAHOO.extend(c, YAHOO.util.DragDrop, {
		...
		_setValue: function(a, b, d, e, g) {
			var j = this.thumb,
			l;
			if (!j.available) return this.deferredSetValue = arguments,
			!1;
		...
		_setRegionValue: function(a, b, d, e, g, j) {
			var l = this.thumb;
			if (!l.available) return this.deferredSetRegionValue = arguments,
			!1;
		...
		verifyOffset: function() {
			var a = e(this.getEl()),
			b = this.thumb; (!this.thumbCenterPoint || !this.thumbCenterPoint.x) && this.setThumbCenterPoint();
			return a && (a[0] != this.baselinePos[0] || a[1] != this.baselinePos[1]) ? (this.setInitPosition(), this.baselinePos = a, b.initPageX = this.initPageX + b.startOffset[0], b.initPageY = this.initPageY + b.startOffset[1], b.deltaSetXY = null, this.resetThumbConstraints(), !1) : !0
		},
		moveThumb: function(a, b, d, k) {
			var g = this.thumb,
			j = this,
			l, n;
			if (g.available) g.setDelta(this.thumbCenterPoint.x, this.thumbCenterPoint.y),
			n = g.getTargetCoord(a, b),
			l = [Math.round(n.x), Math.round(n.y)],
			this.animate && g._graduated && !d ? (this.lock(), this.curCoord = e(this.thumb.getEl()), this.curCoord = [Math.round(this.curCoord[0]), Math.round(this.curCoord[1])], setTimeout(function() {
				j.moveOneTick(l)
			},
			this.tickPause)) : this.animate && c.ANIM_AVAIL && !d ? (this.lock(), a = new YAHOO.util.Motion(g.id, {
				points: {
					to: l
				}
			},
			this.animationDuration, YAHOO.util.Easing.easeOut), a.onComplete.subscribe(function() {
				j.unlock();
				j._mouseDown || j.endMove()
			}), a.animate()) : (g.setDragElPos(a, b), !k && !this._mouseDown && this.endMove())
		},
		_slideStart: function() {
			if (!this._sliding) this._silent || (this.onSlideStart(), this.fireEvent("slideStart")),
			this._sliding = !0,
			this.moveComplete = !1
		},
		_slideEnd: function() {
		...
		moveOneTick: function(a) {
			...
			c = null,
			e;
			b._isRegion ? (c = this._getNextX(this.curCoord, a), e = c !== null ? c[0] : this.curCoord[0], c = this._getNextY(this.curCoord, a), c = c !== null ? c[1] : this.curCoord[1], c = e !== this.curCoord[0] || c !== this.curCoord[1] ? [e, c] : null) : c = b._isHoriz ? this._getNextX(this.curCoord, a) : this._getNextY(this.curCoord, a);
		...
		_getNextX: function(a, b) {
			...
			c = [];
			c = null;
			a[0] > b[0] ? (c = d.tickSize - this.thumbCenterPoint.x, c = d.getTargetCoord(a[0] - c, a[1]), c = [c.x, c.y]) : a[0] < b[0] && (c = d.tickSize + this.thumbCenterPoint.x, c = d.getTargetCoord(a[0] + c, a[1]), c = [c.x, c.y]);
			return c
		},
		...
		fireEvents: function(a) {
			...
			if (!this.isLocked()) if (b._isRegion) {
				a = b.getXValue();
				b = b.getYValue();
				if ((a != this.previousX || b != this.previousY) && !this._silent) this.onChange(a, b),
			...
			b != this.previousVal && !this._silent && (this.onChange(b), this.fireEvent("change", b)),
			this.previousVal = b
		},
		toString: function() {
...
YAHOO.widget.SliderThumb = function(c, e, b, d, a, f, h) {
	if (c) YAHOO.widget.SliderThumb.superclass.constructor.call(this, c, e),
	this.parentElId = e;
	this.isTarget = !1;
	this.tickSize = h;
...
YAHOO.extend(YAHOO.widget.SliderThumb, YAHOO.util.DD, {
	...
	getOffsetFromParent: function(c) {
		var e = this.getEl(),
		b;
		if (this.deltaOffset) b = parseInt(YAHOO.util.Dom.getStyle(e, "left"), 10),
		e = parseInt(YAHOO.util.Dom.getStyle(e, "top"), 10),
		b = [b + this.deltaOffset[0], e + this.deltaOffset[1]];
		else if (b = YAHOO.util.Dom.getXY(e), c = c || YAHOO.util.Dom.getXY(this.parentElId), b = [b[0] - c[0], b[1] - c[1]], c = parseInt(YAHOO.util.Dom.getStyle(e, "left"), 10), e = parseInt(YAHOO.util.Dom.getStyle(e, "top"), 10), c -= b[0], e -= b[1], !isNaN(c) && !isNaN(e)) this.deltaOffset = [c, e];
		return b
	},
	...
	c.prototype = {
		...
		setValues: function(b, a, c, e, k) {
			...
			o = !1,
			m = !1;
			l._isHoriz ? (l.setXConstraint(l.leftConstraint, n.rightConstraint, l.tickSize), n.setXConstraint(l.leftConstraint, n.rightConstraint, n.tickSize)) : (l.setYConstraint(l.topConstraint, n.bottomConstraint, l.tickSize), n.setYConstraint(l.topConstraint, n.bottomConstraint, n.tickSize));
			this._oneTimeCallback(g, "slideEnd",
			function() {
				o = !0;
			...
			this._oneTimeCallback(j, "slideEnd",
			function() {
				m = !0;
		...
		setMinValue: function(b, a, c, e) {
			...
			this.activeSlider = k;
			g = this;
			this._oneTimeCallback(k, "slideEnd",
			function() {
				g.updateValue(e);
				setTimeout(function() {
		...
		updateValue: function(b) {
			...
			j,
			l;
			if (a != this.minVal || c != this.maxVal) e = !0,
			k = this.minSlider.thumb,
			g = this.maxSlider.thumb,
			j = this.isHoriz ? "x": "y",
			l = this.minSlider.thumbCenterPoint[j] + this.maxSlider.thumbCenterPoint[j],
			j = Math.max(c - l - this.minRange, 0),
			l = Math.min( - a - l - this.minRange, 0),
			this.isHoriz ? (j = Math.min(j, g.rightConstraint), k.setXConstraint(k.leftConstraint, j, k.tickSize), g.setXConstraint(l, g.rightConstraint, g.tickSize)) : (j = Math.min(j, g.bottomConstraint), k.setYConstraint(k.leftConstraint, j, k.tickSize), g.setYConstraint(l, g.bottomConstraint, g.tickSize));
			this.minVal = a;
			this.maxVal = c;
			e && !b && this.fireEvent("change", this)
		},
		selectActiveSlider: function(b) {
			var a = this.minSlider,
			c = this.maxSlider,
			e = a.isLocked() || !a.backgroundEnabled,
			k = c.isLocked() || !a.backgroundEnabled,
			g = YAHOO.util.Event;
			e || k ? this.activeSlider = e ? c: a: (b = this.isHoriz ? g.getPageX(b) - a.thumb.initPageX - a.thumbCenterPoint.x: g.getPageY(b) - a.thumb.initPageY - a.thumbCenterPoint.y, this.activeSlider = b * 2 > c.getValue() + a.getValue() ? c: a)
		},
	...
	b.Slider.getVertDualSlider = function(c, a, e, h, k, g) {
		a = new b.SliderThumb(a, c, 0, 0, 0, h, k);
...
YAHOO.util.Attribute.prototype = {
	...
	setValue: function(c, e) {
		...
		if (!e && (b = d.fireBeforeChangeEvent(f), b === !1)) return ! 1;
	...
	YAHOO.util.AttributeProvider.prototype = {
		...
		getAttributeKeys: function() {
			...
			var e = [],
			b;
			for (b in this._configs) c.hasOwnProperty(this._configs, b) && !c.isUndefined(this._configs[b]) && (e[e.length] = b);
			return e
		},
		...
		refresh: function(e, b) {
			for (var d = this._configs = this._configs || {},
			e = (c.isString(e) ? [e] : e) || this.getAttributeKeys(), a = 0, f = e.length; a < f; ++a) d.hasOwnProperty(e[a]) && this._configs[e[a]].refresh(b)
		},
	...
	d.prototype = {
		...
		addListener: function(a, c, d, e) {
			var e = e || this,
			g = YAHOO.util.Event,
			j = this.get("element") || this.get("id"),
			l = this;
			if (b[a] && !g._createMouseDelegate) return ! 1;
			if (!this._events[a]) {
				if (j && this.DOM_EVENTS[a]) g.on(j, a,
				function(b, c) {
		...
		fireQueue: function() {
			...
			b = 0,
			c = a.length; b < c; ++b) this[a[b][0]].apply(this, a[b][1])
		},
		appendTo: function(a, b) {
			...
			var b = b && b.get ? b.get("element") : c.get(b),
			d = this.get("element");
			if (!d) return ! 1;
			if (!a) return ! 1;
		...
		setAttributes: function(a, b) {
			...
			d = this._configOrder,
			e = 0,
			j = d.length; e < j; ++e) a[d[e]] !== void 0 && (c[d[e]] = !0, this.set(d[e], a[d[e]], b));
			for (var l in a) a.hasOwnProperty(l) && !c[l] && this.set(l, a[l], b)
		},
		set: function(a, b, c) {
			var d = this.get("element");
			if (d) return ! this._configs[a] && !YAHOO.lang.isUndefined(d[a]) && this._setHTMLAttrConfig(a),
			e.prototype.set.apply(this, arguments);
			else if (this._queue[this._queue.length] = ["set", arguments], this._configs[a]) this._configs[a].value = b
		},
		...
		_initElement: function(a, b) {
			...
			typeof b.element === "string" && this._setHTMLAttrConfig("id", {
				value: b.element
			});
			c.get(b.element) && (e = !0, this._initHTMLElement(b), this._initContent(b));
			YAHOO.util.Event.onAvailable(b.element,
			function() {
...
function cc(c) {
	...
	c == "" && (e = "1");
	c.length != 6 && (e = "1");
	if (c.match('/[()<>,;:\\/"[]ghijklmnoprstuvzwxyqGHIJKLMNOPRSTUVZQWXY]/')) document.getElementById("startcolor").style.backgroundColor = "#FFFFFF",
	e = "1";
	e != "" && (c = "FFFFFF");
...
YAHOO.util.Color = function() {
	...
	return {
		...
		rgb2hsv: function(b, d, a) {
			...
			d /= 255;
			a /= 255;
			var e, h = Math.min(Math.min(b, d), a),
			k = Math.max(Math.max(b, d), a),
			g = k - h;
			switch (k) {
			case h:
				e = 0;
		...
		websafe: function(b, d, a) {
			...
			var f = function(a) {
				if (e(a)) {
					var a = Math.min(Math.max(0, a), 255),
					b,
					c;
	...
	function c(a, b) {
		e += 1;
		b = b || {};
		arguments.length === 1 && !YAHOO.lang.isString(a) && !a.nodeName && (b = a, a = b.element || null); ! a && !b.element && (a = this._createHostElement(b));
	...
	YAHOO.extend(c, YAHOO.util.Element, {
		...
		IMAGE: {
			PICKER_THUMB: "http://c632769.r69.cf2.rackcdn.com/picker_thumb.png",
			HUE_THUMB: "http://c632769.r69.cf2.rackcdn.com/hue_thumb.png"
		},
		DEFAULT: {
			PICKER_SIZE: 360
		},
		...
		_onPickerSliderChange: function() {
			...
			this.pickerSlider.valueChangeSource !== a.SOURCE_SET_VALUE && this._getValuesFromSliders();
			this._updateFormFields();
			this._updateSwatch()
		},
		...
		_createElements: function() {
			var a, b, c, e, f = this.get(this.OPT.IDS),
			g = this.get(this.OPT.TXT),
			k = this.get(this.OPT.IMAGES),
			h = function(a, b) {
		...
		_initElements: function() {
			...
			e,
			f;
			for (c in this.ID) d.hasOwnProperty(this.ID, c) && (b[this.ID[c]] = b[c]); (e = h.get(b[this.ID.PICKER_BG])) || this._createElements();
		...
		_initSliders: function() {
			...
			this.pickerSlider = a.getSliderRegion(this.getElement(b.PICKER_BG), this.getElement(b.PICKER_THUMB), 0, c, 0, c);
		...
		_bindUI: function() {
			...
			k.on(this.getElement(a.WEBSAFE_SWATCH), "click",
			function() {
				this.setValue(this.get(b.WEBSAFE))
			},
			this, !0);
			k.on(this.getElement(a.CONTROLS_LABEL), "click",
			function(a) {
		...
		initAttributes: function(a) {
			...
			this.setAttributeConfig(this.OPT.ANIMATE, {
				...
				method: function(a) {
					if (this.pickerSlider) this.pickerSlider.animate = a,
...
function deleteChild(c, e) {
	...
	for (var d = 0,
	a = 8,
	d = 1; d <= c; d++) if (b.childNodes[c - d].firstChild.className == "off") {
		a = c - d;
		break
	}
